...
int fit (int const centroidPE, int const roughFWHM) {
    ...
    auto prefitted = hpx->GetFunction("prefitFn");
    prefitted->SetLineColor(kBlue);
    prefitted->Draw("SAME");
    // TEST
    ...
    TFitResultPtr result = hpx->Fit(refitFn, "RS+"); // TEST: Remove the "+" in practice so stats box updates to match second fit
    
    // TEST
    // std::cout << "FUNCTIONS:" << hpx->GetListOfFunctions() << std::endl;
    // TEST
