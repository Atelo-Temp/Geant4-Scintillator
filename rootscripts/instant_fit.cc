...
int fit(std::initializer_list<int> const centroids, int const roughFWHM) {
    ...
    for (int i = 0; i < numPeaks; i++) {
        ...
        double const roughAmplitude = hpx->GetBinContent(centroidVec[i]);
        double const roughMean = hpx->GetXaxis()->GetBinCenter(centroidVec[i]); // get the x-axis location of max counts bin
        double const roughSigma = roughFWHM / SigmaToFWHM;
        
        fitParamsVec.push_back(roughAmplitude);
        fitParamsVec.push_back(roughMean);
        fitParamsVec.push_back(roughSigma);
