...
/*
 * call:
 * plot() - on the spectrum to overlay on another
 * replot() - if needed
 * overlay() 
 * 
 * args:
 * 
 * swap - change which spectrum is overlayed on the other
 */
int overlay(
    std::string const& userPath,
    std::string const& objectName = "",
    ...
    //////
    // 2) Get the secondary plot
    ////
    
    auto overlayHandler = new ROOTHandler();
    
    std::optional<TH1*> result = overlayHandler->plot_root(path, objectName, branchName, nbins, xmin, xmax);
    ...
    TH1* hpx2 = result.value();
    
    
    /////
    // X) Presentation-ready style configuration (TDR/PDG style)
    /////
    ...
    // Smooth curves and legible fonts
    gStyle->SetTextFont(42);
    gStyle->SetLabelFont(42, "XYZ");
    gStyle->SetTitleFont(42, "XYZ");
    
    // Pad dimensions and clean margins
    canvas->SetTicks(1, 1);
    ...
    ///////////////
    // X) Normalisation & cosmetics
    //////////////
    
    // TODO: Toggle between which hpx is front/back eventually
    
    // Since sim has no background and lab is background subtracted, scale sims maximum
    // value cleanly to labs maximum value for comparison
    if (hpx1->GetMaximum() > 0) {
        hpx1->Scale(hpx2->GetMaximum() / hpx1->GetMaximum()); // hpx 1 = sim, hpx2 = lab (hpxSim, hpxLab?)
    }
    
    // Background plot data styling: Dark points with error bars ()
    hpx2->SetMarkerStyle(20);
    hpx2->SetMarkerSize(0.8);
    hpx2->SetMarkerColor(kBlack);
    hpx2->SetLineColor(kBlack);
    hpx2->SetLineWidth(1);
    
    // Overlay plot styling: Smooth, elecgant coloured curve
    hpx1->SetLineColor(kRed + 1); // crimson red
    hpx1->SetLineWidth(3);
    // hpx1->SetFillColorAlpha(kRed, 0.1); // subtle transparent shading under curve
    
    // Axis limits adjustments to avoid clipping peaks
    double maxVal = TMath::Max(hpx1->GetMaximum(), hpx2->GetMaximum());
    // hpx2->GetYaxis()->SetRangeUser(0.0, maxVal * 1.15); // 15% headroom for legend
    hpx2->GetYaxis()->SetRangeUser(0.0, maxVal * 1.05); // 15% headroom for legend
    
    // Axis titles and sizing
    TAxis* hpx2xAxis = hpx2->GetXaxis();
    hpx2xAxis->SetTitle("Channels");
    hpx2xAxis->SetTitleSize(0.05);
    // hpx2xAxis->SetTitleOffset(0.04);
    // hpx2xAxis->SetTitleOffset(0.1);
    // hpx2xAxis->SetTitleOffset(-0.1);
    // hpx2xAxis->SetTitleOffset(0.5);
    
    TAxis* hpx2yAxis = hpx2->GetYaxis();
    hpx2yAxis->SetTitle("Counts");
    hpx2yAxis->SetTitleSize(0.05);
    // hpx2yAxis->SetTitleOffset(1.3);
    // hpx2yAxis->SetTitleOffset(0.5);
    hpx2yAxis->SetLabelSize(0.04);
    // ...
    // NOTE: No y-axis currently
    
    //////////////
    // X) Drawing and annotation
    ////////////
    
    // Draw background spectrum first to set coordinate geometry (lab)
    hpx2->Draw("E1"); // E1: Draw standard error bars
    hpx1->Draw("HIST C SAME"); // hist c: smooth spline curve overlay
    
    // Create a sleek borderless legend
    auto leg = new TLegend(0.60, 0.72, 0.90, 0.88);
    leg->SetBorderSize(0);
    leg->SetFillStyle(0); // transparent background
    leg->SetTextSize(0.04);
    leg->AddEntry(hpx1, "Simulation", "L");
    leg->AddEntry(hpx2, "Lab", "L");
    leg->Draw();
    
    
    /////
    // Canvas update and file compilation out
    ////
...
// plot("../data/152Eu_sim_8500k-event.root")
// replot(2048, 0., 6756.76, true, 0.5, 118., -27.12)
// overlay("../data/152Eu_lab_subtracted.root")
