...
int plot_x(
    ...
    // Ensure flags are reset for consecutive calls (i.e., the if == TH1D logic below will run when
    // loading a ROOT TH1D, THEN, an ASCII file (as ASCII logic doesnt touch the RootObjectType flag))
    if (fileType != FileType::NULLFILE) {
        fileType = FileType::NULLFILE;
    }
    if (rootObjectType != RootObjectType::NULLOBJ) {
