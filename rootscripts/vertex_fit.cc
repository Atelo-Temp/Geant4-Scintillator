...
int replot(
    ...
    // Ensure seed is reset for reproducability (force global generator back to starting state)
    gRandom->SetSeed(SEED);
    // NOTE: Without this, the first execution of replot will always be consistent, but not subequent calls
 ...
 */
int assign_peak_params(TF1* fitFn, std::vector<std::vector<double>> const& fitParamsVec) {
    // ...
    for (int i = 0; i < fitParamsVec.size(); i++) {
        ...
        fitFn->SetParLimits(gausArg0idx, 0., initialAmplitude * 2);
...
int fit(int const view_low, int const view_high, int const numPeaksRequested, double const roughFWHM = 40., std::string const bkgComponent = "pol1") {
    ...
    //////////////////////////////////////
    // 2) Limit window for spectrum search
    //////////////////////////////////////
    
    range(view_low, view_high);
    
    /////////////////////////////
    // 3) Find peaks in the range
    /////////////////////////////
    ...
    std::string const searchOptions = "new nobackground nodraw";
    // NOTE: option:
    // - "nobackground": dont remove background before deconvolution
    // - "goff": supress drawing of red triangles on top of found peaks
    // - "new": employs a more precise, mordern peak-searching algo
    // - "noMarkov": disable Markov chain algorithm
    // - "nodraw": disable drawing of histo with search results
    
    // double const threshold = 0.05; // default
    // double const threshold = 0.01;
    // double const threshold = 0.005; // needed for finding some 152Eu peaks if 122 keV peak is in frame
    ...
    double* const xPositions = spectrum->GetPositionX();
    double* const yPositions = spectrum->GetPositionY();
    ...
    // NOTE: This specific setup matches the background() histogram overload (with 3 args)
    const char* error = spectrum->Background(
        source.data(),
        roiBins,
        numPasses,
        TSpectrum::kBackDecreasingWindow,
    ...
    for (int bin = 1; bin <= numBins; bin++) {
        if ((bin >= view_low) && (bin <= view_high)) {
            ...
            hpxBackground->SetBinContent(bin, 0.);
        }
    }
    ...
    hpxBackground->Draw("SAME");
    
    /////////////////////////////////
    // 5) Fit to the background shape
    /////////////////////////////////
    ...
    if (!isAscending) {
        std::cout << "\nSorting centroid vector...\n";
        // Ensure centroids are in ascending order (i.e. 1100, 1300)
        std::sort(centroidVec.begin(), centroidVec.end());
    }
    
    ////////////////////////////////////////////////////////
    // 7) Determine the amplitude of each gaussian component
    ////////////////////////////////////////////////////////
    
    // NOTE: Subtracts the background height from the total height, leaving the residual amplitude
    
    std::cout << "\nCalculating rough peak amplitudes...\n\n";
    ...
    for (int i = 0; i < numPeaks; i++) {
        ...
        double const peakAmplitude = totalAmplitude - backgroundCounts;
        
        std::cout << "Amplitude #" << i << ": " << peakAmplitude << "\n";
        
        amplitudeVec.push_back(peakAmplitude);
    }
    
    /////////////////////////////////////
    // 8) Determine the FWHM of the peaks
    /////////////////////////////////////
    
    // NOTE:
    // iterate left and right from centroid
    // find bin where residual height = half the amplitude
    // subtract left bin from right bin
    
    std::cout << "\nSearching for rough FWHM values...\n\n";
    ...
    for (int i = 0; i < numPeaks; i++) {
        ...
        // ...
        double const halfMaximum = amplitudeVec[i] / 2.;
        // std::cout << "Half maxmimum: " << i << ": " << halfMaximum << "\n"; // NOTE: for debugging
        
        // ...
        std::vector<double> found = {};
        
        // Go left (j == 0), and right (j == 1), in search of return to half maximum amplitude
        for (int j = 0; j <= 1; j++) {
            // ...
            bool searching = true;
            
            // Start at centroid bin
            int currentBin = xAxis->FindBin(centroidVec[i]);
            
            // std::cout << "Start bin: " << currentBin << "\n"; // NOTE: for debugging
            
            // Iterate left/right until end search condition met
            // while (searching) {
            while (true) {
                // If j is zero, go left, and vice versa
                if (j == 0) currentBin--;
                else if (j == 1) currentBin++;
                ...
                // If we reach the edge of the view window, stop searching
                if ((currentBin <= view_low) || (currentBin >= view_high)) {
                    std::cout << "> Reached edge of search range.\n";
                    std::cout << "> Current bin: " << currentBin << " Hit Bin: " << (currentBin <= view_low ? view_low : view_high) << "\n";
                    // searching = false;
                    break;
                }
                // NOTE: Else would need to handle it going left, not reaching fwhm, but climbing up a peak,
                // returning to the bottom on the other side of it, and calling that fwhm left or right, etc
                ...
                // Found half maximum
                if (residualBinHeight <= halfMaximum) {
                    double const binCentre = hpx->GetBinCenter(currentBin);
                    found.push_back(binCentre);
                    std::cout << "> Found " << (j == 0 ? "left" : "right") << " half maximum.\n";
                    // searching = false;
                    break;
                }
            }
        ...
        if (found.size() == 2) {
            double const peakFWHM = found[1] - found[0]; // subtract the half distance bin on the right [1] from that of the left [0]
            fwhmVec.push_back(peakFWHM);
            
            std::cout << "FWHM #" << i << ": " << peakFWHM << "\n\n";
        }
        else {
            double const peakFWHM = 2 * sqrt(centroidVec[i]);
            fwhmVec.push_back(peakFWHM);
            
            std::cout << "FWHM #" << i << ": " << peakFWHM << " (fallback)\n\n";
        }
    }    
    
    /////////////////////////////////////////////////////////////////////////////
    // 9) Generate a full fit function string based on the number of peaks to fit
    /////////////////////////////////////////////////////////////////////////////
    
    std::string const fitString = fit_string(numPeaks);
    
    std::cout << "\nFit function string: " << fitString << "\n";
    ...
    ////////////////////////////////////////////////////////
    // 9.2) Establish lower/upper bounds for full fit window
    ////////////////////////////////////////////////////////
    ...
    //////////////////////////////////////////////////
    // 9.4) Assign parameters to the full fit function
    //////////////////////////////////////////////////
    
    std::vector<std::vector<double>> fitParamsMatrix = {};
    
    for (int i = 0; i < numPeaks; i++) {
        std::vector<double> fitParamsVec = {};
        
        fitParamsVec.push_back(amplitudeVec[i]);
        fitParamsVec.push_back(centroidVec[i]);
        fitParamsVec.push_back(fwhmVec[i] / SigmaToFWHM);
        
        fitParamsMatrix.push_back(fitParamsVec);
    }
    
    assign_peak_params(fullFitFn, fitParamsMatrix);
    ...
    // NOTE: If calling fit() multiple times, having an initial range that doesnt include
    // the new fits bounds will cause an error
    ...
    // NOTE: If using "+" in the fit commands list below, check its deleted
    // TF1* existingFit = hpx->GetFunction("fullPrefitFn");
    // if (existingFit) {
    //     existingFit->Delete();
    //     gPad->Modified();
    //     gPad->Update();
